...
async function main() {
    ...
    for (let i = 1; i <= initialMintCount; i++) {
        let tx = await TokenHardHat.mintCollectionNFT(signers[0].address, i.toString());
    ...
    let contractOwnerBalances = await TokenHardHat.balanceOf(contractOwner);
    ...
    const owner = await TokenHardHat.ownerOf(NFT1);
    ...
    await TokenHardHat["safeTransferFrom(address,address,uint256)"](contractOwner, collector, NFT1);
    console.log(`${symbol}#${NFT1} transferred from ${contractOwner} to ${collector}`);
    console.log(`Querying the owner of ${symbol}#${NFT1}...`);
    let NFT1Owner = await TokenHardHat.ownerOf(NFT1);
    ...
    const collectorContract = TokenHardHat.connect(signers[1]); 
    ...
    let NFT1Spender = await TokenHardHat.getApproved(NFT1);
    ...
    await TokenHardHat["safeTransferFrom(address,address,uint256)"](collector, contractOwner, NFT1);
    NFT1Owner = await TokenHardHat.ownerOf(NFT1);
    console.log(`Owner of ${symbol}#${NFT1}: ${NFT1Owner}\n`);
    ...
    await TokenHardHat.setApprovalForAll(collector, true) // The second parameter can be set to false to remove operator
    ...
    const approvedForAll = await TokenHardHat.isApprovedForAll(contractOwner, collector);
    ...
    contractOwnerBalances = await TokenHardHat.balanceOf(contractOwner);
    console.log(`BEFORE: ${contractOwner} has ${contractOwnerBalances} NFTs from this ${symbol} collection`);
    let collectorBalances = await TokenHardHat.balanceOf(collector);
    ...
    collectorBalances = await TokenHardHat.balanceOf(collector);
